...
class MPlugin {
    ...
    buildHTMLCombobox(combobox, data) {
        ...
        if (data) {
            this.buildHTMLComboboxData(controlHtml, data, fieldText, fieldValue, me);
        }
     ...
     */
    buildHTMLComboboxData(comboboxHTML, data, fieldText, fieldValue, currentThis) {
        var me = currentThis;
        ...
        $.each(data, function (index, item) {
            ...
            var translatedToVNText = me.translateText(text);

            var itemHTML = `<a class="m-combobox-item" value="` + value + `"><span>` + translatedToVNText + `</span></a>`;
    ...
    /**
     * 
     * Chuyển text của combobox giới tính về dạng tiếng việt
     * @param {any} text chuỗi tiếng anh của combobox giới tính
     * @returns Chuỗi được chuyển sang tiếng việt
     * Author: PMChien
     */
    translateText(text) {
        try {
